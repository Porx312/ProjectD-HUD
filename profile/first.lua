...
local function transport_status_line()
    ...
    return string.format(
        "mode=%s evt=%s wait=%s bundle=%s profile=%s battle=%s battle_state=%s poll_sec=%s synthetic=%s err=%s web=%s steam=%s",
        transport_mode(st),
        ...
        tostring(st.snapshot_poll_sec or "-"),
        st.synthetic_lobby and "y" or "n",
        err ~= "" and err or "-",
        web ~= "" and web or "-",
        steam ~= "" and steam or "-"
    )
